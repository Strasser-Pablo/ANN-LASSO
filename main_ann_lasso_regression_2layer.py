...
for simu in range(num_repli):
    x_train=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'x_train_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    x_val=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'x_val_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    x_test=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'x_test_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    y_train=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'y_train_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    y_val=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'y_val_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    y_test=pd.read_excel(dir_name+truemodel+'/s'+str(s)+'/'+'y_test_'+str(simu)+'.xlsx',header=None,dtype=np.float32)
    ...
    x_train_l2norm=tf.sqrt(tf.reduce_sum(tf.cast(x_train**2,tf.float32),axis=0))
    x_train_l2norm=x_train_l2norm.numpy()
    x_train_rescaled=x_train/np.repeat(x_train_l2norm.reshape(1,p1),n_train,axis=0)
    x_train_rescaled=np.concatenate((x_train_rescaled,np.ones([n_train,1],dtype=np.float32)), axis=1)
    n_test=x_test.shape[0]
    x_test_rescaled=x_test/np.repeat(x_train_l2norm.reshape(1,p1),n_test,axis=0)
    x_test_rescaled=np.concatenate((x_test_rescaled,np.ones([n_test,1],dtype=np.float32)), axis=1)

    lambdaqut_SANN = lambda_qut_sann_regression(tf.nn.l2_normalize(x_train,axis=0),nSample=10000,miniBatchSize=500,alpha=0.05,option='quantile').astype(np.float32)
      
    ipath=-1
    lambi=(np.exp(ipath,dtype=np.float32)/(1+np.exp(ipath,dtype=np.float32))*lambdaqut_SANN).astype(np.float32)
    ...
    if(s==0):
        needles_index_hat=np.array(np.where(np.sum(abs(w1o[:,0:(p1-1)]),axis=0)>0),dtype=np.float32)+1
